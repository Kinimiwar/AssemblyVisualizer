...
namespace AssemblyVisualizer.Controls.Graph
{
		...
		where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
	{
			...
			IDictionary<TVertex, Point> positions, IDictionary<TVertex, Size> sizes)
		{
			...
			return new CompoundLayoutContext<TVertex, TEdge, TGraph>(Graph, positions, sizes, ActualLayoutMode, borders,
				layoutTypes);
		}
		...
		protected virtual void Layout(bool continueLayout)
		{
			...
			var oldPositions = GetOldVertexPositions(continueLayout);
			var oldSizes = GetLatestVertexSizes();
		...
		private IDictionary<TVertex, Point> GetLatestVertexPositions()
		{
			...
			else
			{
				...
				foreach (var vc in _vertexControls)
				{
					var position = vc.Value.TranslatePoint(topLeft, this);
			...
			IDictionary<TVertex, Size> sizes)
		{
			...
			var isValidAlgorithm = OverlapRemovalAlgorithmFactory.IsValidAlgorithm(OverlapRemovalAlgorithmType);
			...
			IDictionary<TVertex, Size> sizes)
		{
			...
			var isValidAlgorithmType = EdgeRoutingAlgorithmFactory.IsValidAlgorithm(EdgeRoutingAlgorithmType);
