...
{
	public abstract class ParameterizedLayoutAlgorithmBase<TVertex, TEdge, TGraph, TVertexInfo, TEdgeInfo, TParam> :
		...
		where TParam : class, ILayoutParameters
	{
		...
		protected override ILayoutIterationEventArgs<TVertex> CreateLayoutIterationEventArgs(int iteration,
			double statusInPercent, string message, IDictionary<TVertex, Point> vertexPositions)
		{
			return new LayoutIterationEventArgs<TVertex, TEdge, TVertexInfo, TEdgeInfo>(iteration, statusInPercent, message,
				vertexPositions, vertexInfos, edgeInfos);
		}
	...
	public abstract class DefaultParameterizedLayoutAlgorithmBase<TVertex, TEdge, TGraph, TParam> :
		ParameterizedLayoutAlgorithmBase<TVertex, TEdge, TGraph, TParam>
		where TVertex : class
		where TEdge : IEdge<TVertex>
		where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
		where TParam : class, ILayoutParameters, new()
	{
		...
		protected DefaultParameterizedLayoutAlgorithmBase(TGraph visitedGraph, IDictionary<TVertex, Point> vertexPositions,
			TParam oldParameters)
			: base(visitedGraph, vertexPositions, oldParameters)
		{
		}
	...
	public abstract class ParameterizedLayoutAlgorithmBase<TVertex, TEdge, TGraph, TParam> :
		LayoutAlgorithmBase<TVertex, TEdge, TGraph>, IParameterizedLayoutAlgorithm<TParam>
		where TVertex : class
		where TEdge : IEdge<TVertex>
		where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
		where TParam : class, ILayoutParameters
	{
		...
		protected virtual ILayoutIterationEventArgs<TVertex> CreateLayoutIterationEventArgs(int iteration,
			double statusInPercent, string message, IDictionary<TVertex, Point> vertexPositions)
		{
		...
		///     Parameters from a prevorious layout. If it is null,
		///     the parameters will be set to the default ones.
		/// </param>
		...
		/// <summary>
		///     Initializes the positions of the vertices. Assign a random position inside the 'bounding box' to the vertices
		///     without positions.
		...
		protected virtual void InitializeWithRandomPositions(double width, double height, double translate_x,
			double translate_y)
		{
